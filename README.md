# GitHub x Figma Mixer

> 3月19日（水）18:30 – 21:00 JST

GitHub と Figma のコラボレーションイベント「GitHub x Figma Mixer」のイベントページです。

## 🎤 登壇者

| 名前 | 役職 | 所属 |
|------|------|------|
| 谷 拓樹 | Designer Advocate | Figma Japan株式会社 |
| William Zhang | Solutions Engineer | GitHub |

## 📋 アジェンダ

| 時間 | 内容 |
|------|------|
| 18:30 – 19:00 | 受付 / Check In |
| 19:00 – 19:05 | 開始のご挨拶 / Opening Remarks |
| 19:30 – 19:40 | FigmaとGitHub Copilotでコードとデザインの意図をつなぐ |
| 19:50 – 20:00 | GitHub最新情報 |
| 20:10 – 20:25 | お楽しみイベント |
| 20:50 – 20:55 | クロージング / Closing |

## 🛠 技術スタック

- [Next.js](https://nextjs.org/) 15.5（App Router、静的エクスポート）
- [Tailwind CSS](https://tailwindcss.com/) 3.4
- TypeScript 5
- GitHub Pages でホスティング（`out/` ディレクトリを GitHub Actions でデプロイ）
- デザインは [Figma MCP](https://www.figma.com/community) を活用して取得

## 📄 ライセンス

MIT
